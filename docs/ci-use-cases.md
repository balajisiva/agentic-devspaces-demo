...
### Sample Review Script - What Actually Happens
...
# Step 1: Get changed files using GitHub MCP (via gh CLI)
echo "Fetching changed files from GitHub..."
...
for FILE in $CHANGED_FILES; do
  ...
  # Step 3: Read file contents using Filesystem MCP
  FILE_CONTENT=$(cat "$FILE")

  # Skip if file is too large (>10KB for API limits)
  FILE_SIZE=$(wc -c < "$FILE")
  if [ $FILE_SIZE -gt 10240 ]; then
    echo "  ⚠ Skipping $FILE (too large: ${FILE_SIZE} bytes)"
    continue
  fi

  # Step 4: Call Anthropic API with file contents
  # This is where ANTHROPIC_API_KEY is actually used!
  REVIEW_JSON=$(curl -s https://api.anthropic.com/v1/messages \
    ...
    -d @- <<EOF
{
  ...
  "messages": [{
    "role": "user",
    "content": "You are a code reviewer. Review this file for:
- Code quality issues
...
Provide a concise review (max 3 bullet points). If the code is good, say 'Looks good ✓'"
  }]
}
EOF
  )
  ...
  AI_REVIEW=$(echo "$REVIEW_JSON" | jq -r '.content[0].text')

  # Step 6: Post review comment using GitHub MCP (via gh CLI)
  if echo "$AI_REVIEW" | grep -q "Looks good"; then
    echo "  ✓ $FILE looks good"
  else
    echo "  💬 Posting review for $FILE"
    gh pr comment $PR_NUMBER --body "**🤖 AI Review: \`$FILE\`**

$AI_REVIEW

---
*Generated by Claude 3.5 Sonnet via Agentic DevSpaces*"
  fi
done

echo "✓ AI code review complete for PR #$PR_NUMBER"
```

**What this script actually does:**

1. **Uses GitHub MCP** (via `gh` CLI) to fetch changed files from the PR
2. **Uses Filesystem MCP** (via `cat`) to read each file's contents
3. **Calls Anthropic API** directly with `ANTHROPIC_API_KEY` - sends file to Claude
4. **Claude analyzes** the code for quality, security, performance issues
5. **Extracts response** from Claude API JSON response
6. **Uses GitHub MCP** (via `gh` CLI) to post review comments on the PR

The MCP servers don't directly call Anthropic - they provide **file access** (filesystem MCP) and **GitHub access** (GitHub MCP). The script uses those + the Anthropic API to create the full review flow.
...
- **Filesystem MCP**: Reads file contents from `/workspace` (provides file access)
- **GitHub MCP**: Fetches PR metadata, posts comments (provides GitHub API access)
- **Anthropic API**: The actual AI that analyzes code (Claude 3.5 Sonnet)
- **Script**: Orchestrates the flow - read files → send to Claude → post results
